        ...
        [
            {
                ...
                maxLimit: number // Max number of suggestions to show for section,
    ...
    <StatefulComponent initialState={{suggestions: []}}>
        {
            ...
            const renderDropdown = (dropdownContent) => <React.Fragment><Button importance="text">Show all</Button>{dropdownContent}</React.Fragment>
            return <Typeahead placeholder="Search by team, artist, event or venue" renderInput={renderInput} onChange={e => getSuggestion(e)} onSelect={onSelect} suggestions={state.suggestions}/>
        }
    }
</StatefulComponent>
</Playground>
