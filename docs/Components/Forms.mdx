    ...
    <StatefulComponent>
        {
            ({state, setState}) => {
                const validate = (output) => {
                    if(output.email !== 'zoidberg@freemail.web') {
                        console.log(output);
                        setState({
                        ...
                        alert("Form submitted" + JSON.stringify(output));
                    }
                };
                return (
                            ...
                            {({ setForm }) => (
                                <React.Fragment>
                                    <TextField ref={setForm}
                                               ...
                                               name='email'
                                               error={state.error}
                                               validationMethod={ value => (!value || value.length <= 10) ? "Input length must be greater than 10" : null}
                                               label="Input Length > 10"/>
                                    <Button type="submit">Submit</Button>
                                </React.Fragment>
